...
var rollMean RollingMean
var rollVar RollingVariance
...
func startMonitor() {
  ...
  go runMonitor(clientset)
}

func stopMonitor() (float64, float64) {
  close(stop)
  return getRollingMean(rollMean), getRollingVariance(rollVar)
}

func runMonitor(clientset *kubernetes.Clientset) {
  for {
    select {
    case <-stop:
      return
    default:
      ...
      rollMean = updateRollingMean(rollMean, cpuRequest.MilliValue())
      rollVar = updateRollingVariance(rollVar, cpuRequest.MilliValue())
      time.Sleep(delay)
    }
  }
}
